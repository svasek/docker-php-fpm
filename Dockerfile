...
RUN apk add --no-cache tzdata libintl libcurl libzip freetype libpng libjpeg-turbo unzip \
      ...
      if [ "$(printf '%s\n' "${PHP_VERSION}" "8.5.0" | sort -V | head -n1)" != "8.5.0" ]; then docker-php-ext-install -j$(nproc) opcache; fi && \
